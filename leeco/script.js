...
function loadCompanyQuestions(company, duration, sort, difficulty) {
  // Map old duration format to new folder structure
  const durationMap = {
    '30days': 'thirty-days',
    '3months': 'three-months',
    '6months': 'six-months',
    '1year': 'more-than-six-months',
    '2year': 'more-than-six-months',
    'alltime': 'all'
  };
  
  const newDuration = durationMap[duration] || duration;
  const csvFile = `data/complete updated data/${company}/${newDuration}.csv`;
  ...
  rows.forEach((row, index) => {
    ...
    cells.forEach((cell, cellIndex) => {
      ...
      if (index === 0 && cellIndex === cells.length - 3) {
        ...
        // Check if user is authenticated and has progress for this problem
        let isChecked = false;
        if (window.userProgress && Array.isArray(window.userProgress)) {
          const progress = window.userProgress.find(p => p.problemId === cells[0]);
          isChecked = progress ? progress.attempted : false;
        } else {
          // Fallback to localStorage for non-authenticated users
          isChecked = JSON.parse(localStorage.getItem(checkbox.id) || "false");
        }
        ...
        checkbox.addEventListener("change", async function () {
          ...
          const problemTitle = cells[2]; // Title is now at index 2
          ...
          if (this.checked) {
            ...
            if (isAuthenticated() && typeof saveProgress === 'function') {
            ...
            if (isAuthenticated() && typeof deleteProgress === 'function') {
              const deleted = await deleteProgress(problemId);
        ...
        // Get date from user progress or localStorage
        let dateValue = "";
        if (window.userProgress && Array.isArray(window.userProgress)) {
          const progress = window.userProgress.find(p => p.problemId === cells[0]);
          dateValue = progress && progress.dateSolved ? formatDate(new Date(progress.dateSolved)) : "";
        } else {
          dateValue = localStorage.getItem(`date-${cells[0]}`) || "";
        }
      ...
      } else if (index > 0 && (cellIndex === 4 || cellIndex === 5)) {
        // Formatting for Acceptance % and Frequency % cells (now at index 4 and 5)
        // Remove the % sign if present and parse
        const value = cell.replace('%', '');
        cellElement.textContent = `${parseFloat(value).toFixed(2)}%`;
      } else {
...
function sortRows(rows, sort, header) {
  ...
  // Map sort keys to match new CSV headers
  let capitalizedSortKey;
  if (sortKey.toLowerCase() === 'frequency') {
    capitalizedSortKey = "Frequency %";
  } else if (sortKey.toLowerCase() === 'acceptance') {
    capitalizedSortKey = "Acceptance %";
  } else {
    capitalizedSortKey = sortKey.charAt(0).toUpperCase() + sortKey.slice(1).toLowerCase();
  }
  ...
  rows.sort((a, b) => {
    ...
    if (capitalizedSortKey === "Frequency %" || capitalizedSortKey === "Acceptance %") {
...
async function loadAndSetIndex() {
  try {
    const response = await fetch("preprocessed_companies.json");
    const data = await response.json();
    searchIndex = data;
    // Convert to array format with problem info
    searchArray = Object.keys(data).map(id => ({
      id: id,
      name: data[id].title || 'Unknown',
      displayText: `${id}. ${data[id].title || 'Unknown'}`,
      difficulty: 'Unknown', // preprocessed_companies.json doesn't have difficulty
      companies: Object.keys(data[id].companies || {})
    }));
    console.log("Search index loaded:", searchArray.length, "problems");
    return searchIndex;
  } catch (error) {
...
function initializeSearchRecommendations() {
  ...
  function createRecommendationItem(problem) {
    ...
    item.className = 'px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center justify-between border-b border-gray-200';
    
    const leftContent = document.createElement('div');
    leftContent.className = 'flex-1 text-black';
    leftContent.textContent = problem.displayText;

    // Only show difficulty if it's not Unknown
    if (problem.difficulty && problem.difficulty !== 'Unknown') {
      const rightContent = document.createElement('div');
      rightContent.className = `text-sm font-semibold ${getDifficultyColor(problem.difficulty)}`;
  ...
  function getDifficultyColor(difficulty) {
    if (!difficulty || difficulty === 'Unknown') return 'text-gray-600';
  ...
  function filterProblems(query) {
    ...
    const results = searchArray.filter(problem => {
      return problem.id.toString().includes(query) || 
